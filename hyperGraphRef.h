...
private:
	...
	void init_hypergraph()
	{
		...
		__vecVisitBool = (bool *)calloc(__numV, sizeof(bool));
		__vecVisitNode = (int *)calloc(__numV, sizeof(int));
...
public:
	...
	std::vector<std::vector<int> > __candsinRR;
	...
	//set candidate set
	void set_candidate_set(std::vector<int> candidates) {
		for (auto &node : candidates) {
			__iscandidate[node] = 1;
		}
	}
	...
	void build_n_RRsets1by1()
	{
		build_one_RRset(gen_random_node_by_weight(__numV, __pAccumWeight), __numRRsets++);
	}

	size_t numVstEdges = 0;
	/// Generate one RR set
	void build_one_RRset(const int uStart, const int hyperIdx)
	{
		...
		//arr[0] = uStart;
		//while (!__que.empty())
		while (idxi < numVisitNode)
		{
			int expand = __vecVisitNode[idxi++];
			if (_cascadeModel == IC)
			{
				for (auto& nbr : _vecGRev[expand])
				{
					int nbrId = nbr.first;
					if (__vecVisitBool[nbrId]) continue;
		...
		for (int i = 0; i < numVisitNode; i++) {
			...
			if (__iscandidate[__vecVisitNode[i]]) {
				cands.push_back(__vecVisitNode[i]);
			}
		}
		__candsinRR.push_back(std::vector<int>(cands.begin(), cands.end()));
		if (__isBilateral) _vecCoverRev.push_back(std::vector<int>(__vecVisitNode, __vecVisitNode + numVisitNode));
	...
	void refresh_hypergraph()
	{
		if (__isBilateral)
		{
			for (int i = 0; i < __numRRsets; i++)
			{
				...
				std::vector<int>().swap(__candsinRR[i]);
			}
			std::vector<std::vector<int>>().swap(_vecCoverRev);
			std::vector<std::vector<int>>().swap(__candsinRR);
		}
